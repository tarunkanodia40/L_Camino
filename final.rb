# <--------------- Q1 --------------->
# v is the input string which has to be parsed 
# returns 1 if parsable else 0
...
# <--------------- Q2 --------------->
# v is the input string which is the lambda term
# returns a pair, first argument is for whether it is parsable
#                 second argument is the list of free variables.
    ...
    if v[0] == '['
        ...
        (1..n-2).each do |i|
            ...
            if v[i]==']' and v[i+1]=='['
                ...
                # else 
                #     printf "TYPE 3=> parsing failed %d(%s), %d(%s)\n",a1,b1,a2,b2
                end
            end
        end 
        return 0,ans
...
# <--------------- Q3 --------------->
# lambda_term is the input string which is the lambda term
# x is the variable to be replaced
# replace_term is the term to which we have to change x
# so it looks like lambda_term[x:=replace_term]
# returns a pair, first argument is for whether it is parsable
#                 second argument is the string with substitution applied
...
# v is the input string which is the lambda term
# returns a pair, first argument is for whether it is parsable
#                 second argument is the string with beta reduction applied
...
def do_beta_reduction(v)
    ...
    if v[0] == '('
        ...
        if a1 == 1
            # printf "Type 2(do_beta_reduction) => %s,%s", v ,v[0..3] + b1 + + v[-1..-1]
            ...
            if v[i]==']' and v[i+1]=='['
                ...
                if a1==1 and a2==1
                    if b1[0]=="("
                        ...
                        # printf "(do_beta_reduction) %s <<<<%s>>> %s \n",v[1..(i-1)],b2,b12
                        if a12 == 1
                            # printf "(do_beta_reduction) %s => %s \n",v[1..(i-1)],b1
...
input = gets.chomp
a1 = parse(input)
printf "Is Parsable(1 is true and 0 is false) == %d",a1
a2,b2= free_vars(input)
printf "\n---Free Vars---\n"
puts b2
a2,b2= substitution(input,"p","(\\x.x)")
printf "\n---substitution---\n"
puts b2
a2,b2= do_beta_reduction(b2)
printf "\n---do_beta_reduction---\n"
puts b2
